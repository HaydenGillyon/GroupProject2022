        ...
        for move in moves:
            ...
            for i, closdmv in enumerate(closed_list):
                if closdmv == move:
                    ...
                    break
            if(not previous):  # if not in closed list
                newBoard = state.node_cost + 1
                present = False
                for j, openmv in enumerate(open_list):# then open list includes move
                    if openmv == move:
                        present = True
...
def take_input():
    r1 = convert_to_int(input().split(" "))
    r2 = convert_to_int(input().split(" "))
    r3 = convert_to_int(input().split(" "))
    return [r1, r2, r3]    
...
start_screen = '''###################################################
###################################################\n
            CHOOSE HEURISTIC TO START              \n
          Press 1 or 2 and enter to begin          \n
###################################################
###################################################\n\n\n'''

enter_start_state = '''\n\n\n
###########################################################################################\n
Please Enter Board Start State by entering each input by a space for a total of 3 per line\n
###########################################################################################\n
'''

enter_goal_state = '''\n\n\n
###########################################################################################\n
Please Enter Board Goal State by entering each input by a space for a total of 3 per line\n
###########################################################################################\n
'''
...
option = input()
option = int(option)
print(enter_start_state)
start = take_input()
print(enter_goal_state)
stop = take_input()
print("\n\n\n\nRunning.....")
result = algo(Node(start,None),Node(stop,None),option)
#result = algo(Node([[1,2,3],[4,5,0],[6,7,8]],None), Node([[0,1,2],[3,4,5],[6,7,8]],None),2)
#result = algo(Node([[7,2,4],[5,0,6],[8,3,1]],None), Node([[0,1,2],[3,4,5],[6,7,8]],None),2)
TotalMoves = 0
